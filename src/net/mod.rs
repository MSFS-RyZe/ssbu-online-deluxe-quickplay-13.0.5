...
unsafe fn main_menu_init(_: &InlineCtx) {
    ...
    // No resetear si estamos en Quickplay activo — las transiciones internas
    // del matchmaking pueden disparar este hook brevemente
    if MATCH_CONNECTION_STATUS.load(Ordering::SeqCst) == MatchConnectionStatus::OnlineQuickPlay as u8 {
        return;
    }
...
#[skyline::hook(offset = 0x22da160, inline)]
...
#[skyline::hook(offset = 0x22da090, inline)]
...
#[skyline::hook(offset = 0x22d9fa0, inline)]
...
/// mid-teardown is the prime suspect for crashes seen on loading screens.
pub fn enforce_ssbusync_arena_mark() {
    if is_online_quickplay_mode() || is_connected() || is_in_real_game() {
